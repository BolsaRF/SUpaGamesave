...
class SaveFinderApp(ctk.CTk):
    ...
    def refresh_profiles_ui(self, prefer_local_results: bool = False):
        ...
        def _worker():
            try:
                ...
                self.after(0, lambda _e=e: self._append_log_text(f"\n[ERROR] Profiles refresh failed: {_e}\n"))
    ...
    def delete_selected_profile_ui(self):
        ...
        profile_name = str(self.selected_profile_name).strip()
        if not profile_name:
            return
        ...
        if self.storage_backend == "local":
            profile_folder = os.path.join(self.local_backups_root, profile_name)
            ...
            def _worker_local():
                try:
                    ...
                    self._append_log_text(f"\n[ERROR] Failed deleting local profile: {e}\n")
                finally:
                    self.after(0, self.refresh_profiles_ui)
        ...
        def _worker_drive():
            try:
                ...
                self.after(0, lambda: self._append_log_text(f"\n[ERROR] Failed deleting Drive profile: {e}\n"))
            finally:
                self.after(0, self.refresh_profiles_ui)
